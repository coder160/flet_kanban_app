from flet import (UserControl,Column,Row,FloatingActionButton,ElevatedButton,Text,GridView,Container,TextField,
                  AlertDialog,Container,icons,border_radius,border,colors,padding,alignment,margin)
...
class Board(UserControl):
    ...
    def __init__(self, app, store: DataStore, name: str):
        ...
        self.app = app
        self.name = name
        self.add_list_button = FloatingActionButton(icon=Iconos.agregar_item, 
                                                    text=Textos.agregar_item, 
                                                    height=Tamanos.agregar_item_altura, 
                                                    on_click=self.create_list)
        self.board_lists = [self.add_list_button]
        for l in self.store.get_lists_by_board(self.board_id):
            self.add_list(l)

        self.list_wrap = Row(self.board_lists,
    ...
    def build(self):
        self.view = Container(content=Column(controls=[self.list_wrap],scroll="auto",expand=True),
                              data=self,
                              margin=margin.all(0),
    ...
    def create_list(self, e):
        option_dict = {colors.LIGHT_GREEN: self.color_option_creator(colors.LIGHT_GREEN),
        ...
        def close_dlg(e):
            if (hasattr(e.control, "text") and not e.control.text == Textos.btn_cancelar) or (type(e.control) is TextField and e.control.value != ""):
        ...
        def textfield_change(e):
            ...
            self.page.update()
        dialog_text = TextField(label=Textos.lista_nueva_nombre,on_submit=close_dlg, on_change=textfield_change)
        create_button = ElevatedButton(text=Textos.btn_crear, bgcolor=colors.BLUE_200, on_click=close_dlg, disabled=True)
        dialog = AlertDialog(title=Text(Textos.dialogo_titulo),
                             content=Column([Container(content=dialog_text,padding=padding.symmetric(horizontal=5)),
                                             color_options,
                                             Row([ElevatedButton(text=Textos.btn_cancelar, 
                                                                 on_click=close_dlg),
                                                                 create_button],
                                                 alignment="spaceBetween")],
                                            tight=True, 
                                            alignment="center"),
                             on_dismiss=lambda e: print(Textos.cerrar_modal))
    ...
    def color_option_creator(self, color: str):
        return Container(bgcolor=color,border_radius=border_radius.all(50),height=10,width=10,
                         padding=padding.all(5),alignment=alignment.center,data=color)
